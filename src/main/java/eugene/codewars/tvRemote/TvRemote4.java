...
/*
    TV Remote (symbols)
    https://www.codewars.com/kata/tv-remote-symbols/java
...
public class TvRemote4 {
    ...
    private static final int COLUMN_LENGTH = LAYOUTS[0].length() / ROW_LENGTH;
    private static final char SHIFT_CHAR = '↑';

    private final String word;

    private Position pos = new Position(0, 0);      // always start from the top-left position
    private int layoutIndex = 0;                    // and lowercase keyboard
    private int length = 0;

    public static int tvRemote(final String word) {
        return new TvRemote4(word).calculatePath();
    }

    private TvRemote4(String word) {
        this.word = word;
    }

    private int calculatePath() {
        for (int i = 0; i < word.length(); i++) {
            pressChar(word.charAt(i));
        }
        return length;
    }
    ...
    private void pressChar(char ch) {
        ...
        if (newPos == null) {
            ...
            // and now try pressing finding that char again
            pressChar(ch);
        } else {
    ...
    private Position findChar(char ch) {
        int index = LAYOUTS[layoutIndex].indexOf(ch);
        if (index < 0) {
            return null;
        }
